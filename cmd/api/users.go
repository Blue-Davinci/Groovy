...
func (app *application) registerUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Copy the data from the request body into a new User struct. We set the
	// Activated field as false.
	user := &data.User{
		Name:      input.Name,
		Email:     input.Email,
		Activated: false,
	...
	// We use a goroutine which runs an anonymous function that sends it.
	// Use the background helper to execute the anonymous function that sends the welcome
	// email.
	app.background(func() {
	...
	// Seend the client a 202 Accepted status code.
